...
switch(calendar.week)
{
	...
	case 2:
		Oled_ShowString(1, 11, "Tues ");
	break;

	case 3:
		Oled_ShowString(1, 11, "Wed ");
	break;

	case 4:
		Oled_ShowString(1, 11, "Thur");
	break;

	case 5:
		Oled_ShowString(1, 11, "Fri ");
	break;

	case 6:
		Oled_ShowString(1, 11, "Sat ");
	break;
}
...
如果需要修改时间，需要改变BKP_ReadBackupRegister(BKP_DR1) != 0x5050和BKP_WriteBackupRegister(BKP_DR1, 0X5050);	这两个语句中的0x5050，修改一样
另外，修改RTC_Set(2020,12,31,23,59,50);  //设置时间
返回值：1：初始化时钟失败,晶振有问题	
				0：初始化正常	
*****/
uint8_t RTC_Init(void)
{
	...
	{	 			
		...
		RTC_WaitForLastTask();																		//等待最近一次对RTC寄存器的写操作完成
		RTC_Set(2024,12,12,20,00,0);  														//设置时间	
